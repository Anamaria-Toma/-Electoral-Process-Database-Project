...
ALTER TABLE alegere
    ADD CONSTRAINT alegeri_tip_ck
        CHECK ( tip_alegere IN ('Locale', 'Parlamentare', 'Parlamentul European', 'Preziden?iale', 'Referendum'));
...
FOR EACH ROW
BEGIN
    -- Verificãm dacã ID-ul nu a fost setat ?i îl generãm automat din secven?ã
...
ALTER TABLE circumscriptie
    ADD CONSTRAINT circumscriptie_nume_ck
        CHECK (nume IN ('Alba', 'Arad', 'Arge?', 'Bacãu', 'Bihor', 'Bistri?a-Nasaud', 'Boto?ani', 'Bra?ov', 'Brãila', 'Bucure?ti', 'Buzãu', 'Cara?-Severin', 'Cluj', 'Constan?a', 'Covasna','Craiova', 'Cãlãra?i', 'Diaspora', 'Dolj', 'Dâmbovi?a', 'Gala?i', 'Giurgiu', 'Gorj', 'Harghita', 'Hunedoara', 'Ialomi?a', 'Ia?i', 'Ilfov', 'Maramure?', 'Mehedin?i', 'Mure?', 'Neam?', 'Olt', 'Prahova', 'Satu-Mare', 'Sibiu', 'Suceava', 'Sãlaj', 'Teleorman', 'Timi?', 'Tulcea', 'Vaslui', 'Vrancea', 'Vâlcea'));
ALTER TABLE circumscriptie
...
DECLARE
    v_count NUMBER;
BEGIN
    -- Comparã doar data alegerii (fãrã ora) cu data votului (fãrã ora)
    SELECT COUNT(*)
    INTO v_count
    FROM alegere a
    ...
    -- Dacã nu gãse?te o alegere corespunzãtoare, ridicã eroare
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Data votului trebuie sã fie aceea?i cu data alegerii!');
    END IF;
END;
/
